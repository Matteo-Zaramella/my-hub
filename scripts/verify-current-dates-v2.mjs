...
async function verifyChallenges() {
  ...
  for (const correct of correctDates.challenges) {
    ...
    // Le sfide usano start_date come data principale
    if (!dbChallenge.start_date) {
      console.log(`❌ SFIDA ${correct.number} - start_date è NULL`)
    ...
    let dbDate
    try {
      dbDate = new Date(dbChallenge.start_date).toISOString()
    } catch (e) {
      console.log(`❌ SFIDA ${correct.number} - DATA INVALIDA NEL DB`)
      console.log(`   Nel DB:    ${dbChallenge.start_date} (NON VALIDA)`)
      console.log(`   Corretta:  ${correct.date}\n`)
      errorsFound++
      continue
    }
    ...
    if (dbDate !== correctDate) {
      ...
      const diffDays = Math.abs(new Date(dbDate) - new Date(correctDate)) / 1000 / 60 / 60 / 24
      console.log(`   Differenza: ${diffDays.toFixed(1)} giorni\n`)
      errorsFound++
    } else {
...
async function verifyClues() {
  ...
  if (clueError) {
    console.error('❌ Errore lettura indizi:', clueError.message)
    return
  }
  ...
  for (const [challengeNum, clueDates] of Object.entries(correctDates.clues)) {
    ...
    if (!challenge) {
      console.log(`❌ SFIDA ${challengeNumber} NON TROVATA NEL DATABASE`)
      errorsFound += clueDates.length
      continue
    }
    ...
    for (let i = 0; i < clueDates.length; i++) {
      ...
      if (!dbClue.revealed_date) {
        console.log(`❌ INDIZIO ${i + 1} - revealed_date è NULL`)
        console.log(`   Corretta:  ${correctDate}`)
        errorsFound++
        continue
      }
      ...
      let dbDate
      try {
        dbDate = new Date(dbClue.revealed_date).toISOString()
      } catch (e) {
        console.log(`❌ INDIZIO ${i + 1} - DATA INVALIDA NEL DB`)
        console.log(`   Nel DB:    ${dbClue.revealed_date} (NON VALIDA)`)
        console.log(`   Corretta:  ${correctDate}`)
        errorsFound++
        continue
      }
...
async function checkCluesAfterChallenges() {
  ...
  if (!clues || !challenges) {
    console.log('❌ Impossibile verificare')
    return 0
  }
  ...
  for (const clue of clues) {
    ...
    if (!challenge || !clue.revealed_date || !challenge.start_date) continue

    const clueDate = new Date(clue.revealed_date)
    const challengeDate = new Date(challenge.start_date)

    if (clueDate >= challengeDate) {
      console.log(`❌ CRITICO: Indizio ${challenge.challenge_number}.${clue.clue_number} DOPO la sfida!`)
